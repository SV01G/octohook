...
-- ══════════════════════════════════════════════════
--  PALETTE  (matches screenshot exactly)
-- ══════════════════════════════════════════════════
local C = {
    BG        = Color3.fromRGB(30,  30,  30),  -- main window bg
    SIDEBAR   = Color3.fromRGB(46,  46,  46),  -- sidebar bg
    PANEL     = Color3.fromRGB(34,  34,  34),  -- content panels
    PANEL2    = Color3.fromRGB(38,  38,  38),  -- element bg
    BORDER    = Color3.fromRGB(45,  45,  45),  -- subtle borders
    GREEN     = Color3.fromRGB(15, 125,  45),  -- header bars, active tab
    GREEN_TXT = Color3.fromRGB(30, 145,  65),  -- button text, active highlights
    GREEN_CHK = Color3.fromRGB(20, 145,  50),  -- checked checkbox
    BTN_BG    = Color3.fromRGB(31,  31,  31),  -- dark button bg
    BTN_HOV   = Color3.fromRGB(38,  38,  38),  -- button hover
    TEXT      = Color3.fromRGB(215, 215, 215), -- normal text
    TEXT_DIM  = Color3.fromRGB(135, 135, 135), -- muted text
    TEXT_LOGO = Color3.fromRGB(220, 255, 220), -- logo text
...
local function DoNotify(title, msg, duration)
    ...
    local frame = New("Frame", {
        Size             = UDim2.new(1, 0, 0, msg and 40 or 24),
...
function LuckyCharms:CreateWindow(cfg)
    ...
    local W         = (cfg.Size and cfg.Size.W) or 600
    local H         = (cfg.Size and cfg.Size.H) or 410
    local SIDEBAR_W = 76
    ...
    -- ── Main frame (sharp corners, 1px border) ───────
    ...
    function Window:AddTab(name)
        ...
        -- Sidebar tab button (32px tall, full width)
        ...
        -- Page (plain scrolling frame, no rounded corners)
        ...
        -- Green header bar (full-width, like "Settings" / "User Interface" in screenshot)
        ...
        -- ── Internal element frame ───────────────────
        -- All elements go inside a shared flat dark panel.
        -- For grouped layout, caller uses Section() then individual elements.
        -- Each element is just a row inside the Page.
        ...
        -- ── TOGGLE ───────────────────────────────────
        -- Small 11×11 square checkbox on the left, label to its right
        ...
        function Tab:Slider(cfg)
            ...
            -- two-row element: label+value on top, track on bottom
            local f = New("Frame", {
            ...
            local fmt = decimal > 0 and ("%."..tostring(math.ceil(-math.log10(decimal))).."f") or "%d"
        ...
        function Tab:Dropdown(cfg)
            ...
            -- header row (dark bg, green text arrow)
            ...
            local listH = #options * LIST_ITEM_H + 2
                ...
                ob.MouseButton1Click:Connect(function()
                    if multi then
                        ...
                        if optBtns[selected] then
                            optBtns[selected].lbl.TextColor3 = C.TEXT_DIM
                        end
                        selected = opt
            ...
            headerBtn.MouseButton1Click:Connect(function()
                ...
                if isOpen then
                    ...
                    wrap.Size = UDim2.new(1, 0, 0, 23 + listH)
                    arrow.Text = "▴"
                else
                    optFrame.Visible = false
                    wrap.Size = UDim2.new(1, 0, 0, 22)
        ...
        -- ── COLORPICKER ──────────────────────────────
        -- Shows as a small colored swatch; click to open full picker
        function Tab:Colorpicker(cfg)
            ...
            -- swatch preview (flat rectangle, no rounded corners)
            ...
            -- Picker popup (200×180, sharp corners)
            local PICKER_W, PICKER_H = 200, 185
            ...
            -- SV square
            local SV_W = PICKER_W - 30  -- leaves room for hue+alpha bars on right
            local svBox = New("Frame", {
                Size             = UDim2.fromOffset(SV_W - 12, 110),
            ...
            local satOverlay = New("Frame", {
                Size = UDim2.new(1,0,1,0), BackgroundColor3 = C.WHITE, BorderSizePixel = 0,
            }, svBox)
            New("UIGradient", {
            ...
            local valOverlay = New("Frame", {
                Size = UDim2.new(1,0,1,0), BackgroundColor3 = C.BLACK, BorderSizePixel = 0,
            }, svBox)
            New("UIGradient", {
                Rotation = 90,
            ...
            -- Hue bar (right side, vertical)
            local hueBar = New("Frame", {
                AnchorPoint      = Vector2.new(1, 0),
                Position         = UDim2.new(1, 0, 0, 0),
                Size             = UDim2.fromOffset(11, 110),
                BorderSizePixel  = 0,
            }, picker)
            ...
            -- Alpha bar (horizontal, below SV)
            local alphaBar = New("Frame", {
                Position         = UDim2.fromOffset(0, 116),
                Size             = UDim2.fromOffset(SV_W - 12, 10),
                BackgroundColor3 = C.WHITE,
                BorderSizePixel  = 0,
            }, picker)
            ...
            local hexBox = New("TextBox", {
                ...
                Size             = UDim2.fromOffset(SV_W - 12, 18),
                BackgroundColor3 = Color3.fromRGB(48, 48, 48),
                BorderSizePixel  = 0,
            ...
            local function Refresh()
                ...
                alphaGrad.Color         = ColorSequence.new{ ColorSequenceKeypoint.new(0, col), ColorSequenceKeypoint.new(1, col) }
            ...
            local function MkDragBtn(target, onDown)
                local b = New("TextButton", {
                    Size = UDim2.new(1,0,1,0), BackgroundTransparency=1,
                    Text="", BorderSizePixel=0, ZIndex=31,
                }, target)
            ...
            MkDragBtn(svBox,    function() dragSV    = true end)
            MkDragBtn(hueBar,   function() dragHue   = true end)
            MkDragBtn(alphaBar, function() dragAlpha = true end)
            ...
            UserInputService.InputBegan:Connect(function(inp, gpe)
                ...
                if inp.UserInputType == Enum.UserInputType.MouseButton1 then
                    ...
                    local sa = swatchWrap.AbsolutePosition; local ss = swatchWrap.AbsoluteSize
                    local inPicker = mp.X>=pa.X and mp.X<=pa.X+ps.X and mp.Y>=pa.Y and mp.Y<=pa.Y+ps.Y
                    local inSwatch = mp.X>=sa.X and mp.X<=sa.X+ss.X and mp.Y>=sa.Y and mp.Y<=sa.Y+ss.Y
                    if not inPicker and not inSwatch then
                        isOpen = false; picker.Visible = false
                    end
                end
            end)
            ...
            LuckyCharms.Callbacks[flag] = function(col, a)
                h, s, v = col:ToHSV(); alpha = a or alpha; Refresh()
            end
        ...
        function Tab:Keybind(cfg)
            ...
            -- key badge (dark bg, dim text, like "INS" in screenshot)
            ...
            UserInputService.InputBegan:Connect(function(inp, gpe)
                ...
                if inp.UserInputType == Enum.UserInputType.Keyboard then
                    if inp.KeyCode == Enum.KeyCode.Escape then
                        badge.Text = currentKey and tostring(currentKey):gsub("Enum%.KeyCode%.","") or "NONE"
                        badge.TextColor3 = C.TEXT_DIM
                        listening = false
                        return
                    end
            ...
            LuckyCharms.Callbacks[flag] = function(k)
                ...
                badge.Text = k and tostring(k):gsub("Enum%.KeyCode%.",""):gsub("Enum%.UserInputType%.","") or "NONE"
                badge.TextColor3 = C.TEXT_DIM
                callback(k)
            end
        ...
        function Tab:Textbox(cfg)
            ...
            -- Two-row element: label on top row, input on bottom row
            local f = New("Frame", {
        ...
        -- ── BUTTON ───────────────────────────────────
        -- Dark bg, green text — exactly as in the screenshot (Load / Save / Reset)
...
function LuckyCharms:SetFlag(f, v)
    local cb = LuckyCharms.Callbacks[f]
    if cb then cb(v) end
end
